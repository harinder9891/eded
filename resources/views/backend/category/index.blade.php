						...
						<div class="table-responsive border-top">
							<table class="table table-bordered table-hover mb-0 text-nowrap display nowrap" id="table">
								...
								<tbody>
									...
									<tr>
										<td><input type="checkbox" class='check' id='check' value="{{$category->id}}"></td>
										<td>{{$category->category}}</td>
										<td>
											<div class="d-flex">
												<img class="avatar avatar-md  d-block brround cover-image mr-3" src={{$category->image}}>
											</div>
										</td>
										<td>{{ $category->description}}</td>
										<td>{{ $category->status == 0 ? "Inactive":"Active"}}</td>
										<td>
											<a href={{ url('/admin/categories/'.$category->id.'/edit')}} class="btn btn-success btn-sm text-white" data-toggle="tooltip" data-original-title="Edit"><i class="fa fa-pencil"></i></a>
